...
pasta_livros="./biblia_NVA" # nome da pasta onde vai ficar os livros .html
usuario=usuario
senha=senha
...
#exit;
	for linha in $verso_bruto; do
		...
		n_livro="${BASH_REMATCH[1]}"
		n_capitulo="${BASH_REMATCH[2]}"
		n_versiculo="${BASH_REMATCH[3]}"
		versiculo="${BASH_REMATCH[5]//\'/\\\'}" # escapo as aspas simples
		versiculo=${versiculo##+( )} #Remove longest matching series of spaces from the front ->  Remova a série de espaços correspondentes mais longa da frente
